...
function CampaignDetailsPage() {
    ...
    const visibleMonths = sliceMonths(months, {
        startDate,
        endDate
    });
    const visibleWeeks = sliceWeeks(weeks, {
        startDate,
        endDate
    });
    const visiblePatientsAttended = sliceMonths(data.patientsAttended, {
        startDate,
        endDate
    });
    const visibleRevenueBars = sliceWeeks(data.revenueBars, {
        startDate,
        endDate
    });
    const visibleCostSeries = sliceWeeks(data.costSeries, {
        startDate,
        endDate
    });
    const visibleMetrics = data.metrics.map((metric)=>({
            ...metric,
            value: metric.tone === "success" ? metric.value : scaleDisplayValue(metric.value, monthRatio)
        }));
    const visibleChannels = data.channelBreakdown.map((channel)=>({
            ...channel,
            amount: scaleDisplayValue(channel.amount, monthRatio)
        }));
    const visibleDropoff = data.dropoff.map((item)=>({
            ...item,
            count: scaleDisplayValue(item.count, monthRatio),
            revenue: scaleDisplayValue(item.revenue, monthRatio)
        }));
    const visibleEntitySummary = {
        entities: scaleDisplayValue(data.entitySummary.entities, monthRatio),
        enrolled: scaleDisplayValue(data.entitySummary.enrolled, monthRatio),
        revenue: scaleDisplayValue(data.entitySummary.revenue, monthRatio)
    };
    const visibleEntitySeries = data.entitySeries.map((item)=>({
            ...item,
            bar: Number(scaleDisplayValue(String(item.bar), monthRatio).replace(/,/g, "")),
            line: Number(scaleDisplayValue(String(item.line), monthRatio).replace(/,/g, ""))
        }));
    const visibleRevenueCurrent = scaleDisplayValue(`Ð ${data.revenueCurrent}K`, monthRatio);
    const visibleRevenueTarget = scaleDisplayValue(`Ð ${data.revenueTarget}K`, monthRatio);
    const visibleCostCurrent = scaleDisplayValue(data.costCurrent, monthRatio);
    const visibleCostTarget = scaleDisplayValue(data.costTarget, monthRatio);
...
//# sourceMappingURL=_13~bxm7._.js.map